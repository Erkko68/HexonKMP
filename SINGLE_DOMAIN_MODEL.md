...
Complete class-level model of the server and shared KMP modules in a single diagram, grouped by domain.
...
classDiagram
    ...
    namespace SharedGame {
        ...
        class Board {
            ...
            +placeVertexBuilding(typeId: BuildingId, ownerId: String, h1: HexCoord, h2: HexCoord, h3: HexCoord, checkConnection: Boolean) Boolean
            +placeEdgeBuilding(typeId: BuildingId, ownerId: String, h1: HexCoord, h2: HexCoord) Boolean
            +getBuildingAt(locId: String) PlacedBuilding?
            +canPlaceVertexBuilding(ownerId: String, h1: HexCoord, h2: HexCoord, h3: HexCoord, targetTypeId: BuildingId, checkConnection: Boolean) Boolean
            +canPlaceEdgeBuilding(ownerId: String, h1: HexCoord, h2: HexCoord, targetTypeId: BuildingId) Boolean
            +getAvailableVertexPlacements(ownerId: String, buildingId: BuildingId, checkConnection: Boolean) List~Triple~
            +getAvailableEdgePlacements(ownerId: String, buildingId: BuildingId) List~Pair~
    ...
    namespace WSMessages {
        ...
        class LobbyEvent {
            <<sealed - server to client>>
            LobbySnapshot(lobbyId, players, maxPlayers, colors)
            PlayerJoined(lobbyPlayer: LobbyPlayer)
            PlayerLeft(playerId: String)
            PlayerUpdated(lobbyPlayer: LobbyPlayer)
            GameStarted
            LobbyError(errorMessage: String, code: LobbyErrorCode)
        }
        class GameplayIntent {
            <<sealed - client to server>>
            EndTurn
            Build(buildingId, h1, h2, h3?)
            MoveRobber(hexA: HexCoord)
            ProposeTrade(give: Map~ResourceId,Int~, want: Map~ResourceId,Int~)
            RespondToTrade(offererId: PlayerId, accepted: Boolean)
            ConfirmTrade(responderId: PlayerId)
            CancelTrade(offererId: PlayerId)
            ExchangeWithBank(give: Map~ResourceId,Int~, get: Map~ResourceId,Int~)
        }
        class GameplayEvent {
            <<sealed - server to client>>
            GameConfigLoaded(config: GameConfig)
            GameSnapshot(board, players, turnId, dice, robber)
            GamePlayerStats(player: GamePlayer)
            DiceRolled(values: Pair~Int, Int~)
            TurnChanged(turnPhase: TurnPhase, newPlayerId: PlayerId)
            ObjectBuilt(playerId, buildingId, h1, h2, h3?)
            RobberUpdated(location: HexCoord)
            ResourcesUpdated(changes: Map~ResourceId,Int~, reason: UpdateReason)
            ResourceCountUpdated(playerId: PlayerId, changes: Int, reason: UpdateReason)
            TradeProposed(give, want, offererId)
            TradeResponse(offererId, responderId, accepted)
            TradeCompleted(responderId, offererId)
            TradeCancelled(offererId: PlayerId)
            GameError(message: String, code: GameErrorCode)
            GameEnded(winnerId: String?, gameId: String)
        }
    }
    ...
    AccountVerificationServiceImpl --> ProfileRepository
    ...
    SocialServiceImpl --> AuthRepository
    ...
    GameEngineImpl --> Board
    GameEngineImpl --> GamePlayer
    GameEngineImpl --> GameConfig
    GameEngineImpl --> TradeOffer
    GameEngineImpl --> TurnPhase
