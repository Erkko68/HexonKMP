...
class VerifyRoutesEmailTest {
    ...
    @Test
    fun `verify returns INVALID_EMAIL if email has no at sign`() =
    ...
    @Test
    fun `verify returns INVALID_EMAIL if email has no domain`() =
        withTestAuthClient { client, inBox ->
            val body = client.verify(
                email = "alice@",
                code = "123456"
    ...
    @Test
    fun `verify returns INVALID_EMAIL if email contains emoji`() =
        withTestAuthClient { client, inBox ->
            val body = client.verify(
                email = "alice😀@test.com",
                code = "123456"
    ...
    @Test
    fun `verify returns INVALID_EMAIL if email exceeds max length`() =
        withTestAuthClient { client, inBox ->
            val longEmail =
