...
class RegisterRoutesUsernameTest {
    ...
    @Test
    fun `register returns INVALID_USERNAME if username is empty`() =
    ...
    @Test
    fun `register returns INVALID_USERNAME for malformed username`() =
        withTestAuthClient { client, inBox ->
    ...
    @Test
    fun `register returns INVALID_USERNAME if username is too long`() =
        withTestAuthClient { client, inBox ->
            val body =
                client.register("AnUserNameShouldntBeThisLong", "alice@test.com", "Secret123!")
            assertEquals(RegisterResult.INVALID_USERNAME, body.result)
        }

    @Test
    fun `register returns USERNAME_EXISTS if username is taken by another unverified user`() =
        withTestAuthClient { client, inBox ->
