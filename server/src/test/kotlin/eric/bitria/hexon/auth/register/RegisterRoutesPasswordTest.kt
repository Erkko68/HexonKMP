...
class RegisterRoutesPasswordTest {
    ...
    @Test
    fun `register returns INVALID_PASSWORD if password is too long`() =
    ...
    @Test
    fun `register returns INVALID_PASSWORD for weak password`() =
        withTestAuthClient { client, inBox ->
    ...
    @Test
    fun `register returns INVALID_PASSWORD if password is empty`() =
        withTestAuthClient { client, inBox ->
    ...
    @Test
    fun `register accepts password with allowed special characters`() =
        withTestAuthClient { client, inBox ->
            val body = client.register(
                "alice",
                "alice@test.com",
